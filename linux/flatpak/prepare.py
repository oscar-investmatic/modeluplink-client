...
paths = subprocess.check_output(["git", "ls-tree", "-rz", "--name-only", "HEAD"], cwd=root).split(
    b"\0"
)
for raw in paths:
    if not raw:
...
(output / "BUILD_DATE").write_text(
    datetime.datetime.fromtimestamp(info["source_date_epoch"], datetime.timezone.utc)
    .date()
    .isoformat()
    + "\n"
)
