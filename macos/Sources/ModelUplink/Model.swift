...
@MainActor final class UplinkModel: ObservableObject {
    ...
    @Published var paused: Set<String> = Set(
        UserDefaults.standard.stringArray(forKey: "pausedConnections") ?? [])
    ...
    var connected: Bool {
        !trialFinished
            && endpoints.contains {
                $0.online && !paused.contains($0.slug)
                    && (localSources[$0.slug] == nil
                        || ["server_available", "ready"].contains(localSources[$0.slug]!.status))
            }
    }
    var working: Bool { busy || loading }
    var paid: Bool { account?.billing_state == "active" || account?.billing_state == "grace" }
    var trialFinished: Bool {
        !paid && (account?.trial_status == "expired" || account?.trial_status == "exhausted")
    }
    var allowance: String {
        if account?.billing_state == "active" || account?.billing_state == "grace" {
        ...
        if account?.trial_status == "active" {
            ...
            return
                "\(account?.trial_remaining ?? 0) free requests · \(String(format: "%.1f", megabytes)) MB left"
        }
    ...
    func request(_ fields: [String: String]) async -> Reply? {
        do {
            ...
            self.error =
                (error as? AppError)?.localizedDescription
    ...
    func apply(_ reply: Reply) {
        ...
        if otherTrial?.id != reply.other_trial?.id {
            keepingOtherComputer = false; preparingMove = false
        }
        ...
        if endpointName.isEmpty, let suggestion = nameSuggestions.first {
            endpointName = suggestion
        }
        sharedModels = (reply.shared_models ?? [:]).compactMapValues { $0 }
        activity = reply.activity ?? [:]
        localSources = reply.local_sources ?? [:]
        if let stopped = reply.stopped {
            for (slug, value) in stopped {
                if value { paused.insert(slug) } else { paused.remove(slug) }
            }
        }
        memoryReleasePending = Set(
            (reply.memory_release_pending ?? [:]).filter { $0.value }.map { $0.key })
    ...
    func inspectSource(_ action: String) async {
        ...
        guard
            let reply = await request([
                "action": action, "local_url": localURL, "local_key": localKey, "model": localModel,
            ])
        else { return }
        if action == "discover_servers" {
            ...
            sourceMessage =
                servers.isEmpty
                ? "No local APIs found. Open your runtime app and enable its API server."
                : "Select a detected API address, or enter your own."
        }
    ...
    func modelsFor(_ endpoint: Endpoint) -> [String] {
        localSources[endpoint.slug]?.models ?? models
    }
    func connectionStatus(_ endpoint: Endpoint) -> String {
        ...
        if let source = localSources[endpoint.slug],
            !["server_available", "ready"].contains(source.status)
        {
            return "LOCAL SERVER NEEDS ATTENTION"
        }
        return "ONLINE"
    }
    func friendAccess() {
        NSWorkspace.shared.open(URL(string: "https://modeluplink.com/dashboard/#api-keys")!)
    }
    func sendCode() async {
        guard !working else { return }
        revision += 1
        ...
        defer { busy = false; message = "" }
        if let reply = await request(["action": "start_code", "email": email]) {
            challenge = reply.challenge_id; code = ""
        }
    }
    func verify() async {
        ...
        defer { busy = false; message = "" }
        if let reply = await request([
            "action": "verify_code", "challenge_id": challenge, "code": code,
        ]) {
    ...
    func connect(moveTrialID: String? = nil) async {
        ...
        receive(
            SetupProgress(event: "progress", stage: "account", message: "Checking your account…"))
        defer { busy = false; connecting = false; message = "" }

        var fields = ["action": "connect", "model": selectedModel]
        if !managedSetup && !localModel.isEmpty {
            fields["local_url"] = localURL; fields["local_key"] = localKey;
            fields["model"] = localModel
        }
        if paid { fields["endpoint_name"] = endpointName }
        if let moveTrialID {
            fields["action"] = "move_trial"; fields["move_trial_id"] = moveTrialID
        }
        ...
        if let endpoint = reply.endpoint {
            ...
            otherTrial = nil; connectionLimit = nil; preparingMove = false;
            keepingOtherComputer = false
            if let key = reply.api_key { saveKey(key, for: endpoint.id) }
        }
        if let notice = reply.notice { error = notice }
        if let shared = reply.shared_models {
            sharedModels.merge(shared.compactMapValues { $0 }) { _, new in new }
        }
    }
    func act(_ action: String, endpoint: Endpoint) async {
        ...
        revision += 1
        busy = true; error = "";
        message = action == "resume" ? "Reconnecting…" : "Updating your connection…"
        defer { busy = false; message = "" }
        guard let reply = await request(["action": action, "slug": endpoint.slug]) else { return }
        if action == "pause" {
            paused.insert(endpoint.slug)
            if reply.memory_release_pending?[endpoint.slug] == false {
                retired.removeAll { $0.id == endpoint.id }
            }
            if reply.memory_release_pending?[endpoint.slug] == true {
                memoryReleasePending.insert(endpoint.slug)
            } else {
                memoryReleasePending.remove(endpoint.slug)
            }
        }
        if let notice = reply.notice { error = notice }
        if action == "resume" {
            paused.remove(endpoint.slug); memoryReleasePending.remove(endpoint.slug)
        }
        if action == "delete" {
            paused.remove(endpoint.slug); KeyStore.remove(endpoint.id);
            memoryKeys.removeValue(forKey: endpoint.id)
    ...
    func updateSharing(_ endpoint: Endpoint, models: [String]) async -> Bool {
        ...
        guard let data = try? JSONEncoder().encode(models),
            let modelIDs = String(data: data, encoding: .utf8)
        else { return false }
        guard
            let reply = await request([
                "action": "share_models", "slug": endpoint.slug, "model_ids": modelIDs,
            ])
        else { return false }
        if let shared = reply.shared_models {
            sharedModels.merge(shared.compactMapValues { $0 }) { _, new in new }
    ...
    func copyKey(_ endpoint: Endpoint) async {
        if let key = memoryKeys[endpoint.id] ?? KeyStore.read(endpoint.id) {
            copy(key, label: "API key")
        } else {
            await act("new_key", endpoint: endpoint)
        }
    }
    ...
    func copy(_ text: String, label: String) {
        ...
        copied = "\(label) copied"
        Task {
            try? await Task.sleep(for: .seconds(3)); if copied == "\(label) copied" { copied = "" }
        }
    }
    func signOut() async {
        guard !working else { return }
        ...
        busy = true; error = ""
        defer { busy = false }
        if await request(["action": "sign_out"]) != nil {
            localKey = ""; testedSource = ""; account = nil; endpoints = []; retired = [];
            otherTrial = nil; connectionLimit = nil; preparingMove = false;
            keepingOtherComputer = false; memoryKeys = [:]; challenge = nil; code = ""
        }
    }
    func setStartup(_ enabled: Bool) async {
        ...
        if let reply = await request(["action": "startup", "enabled": enabled ? "true" : "false"]),
            let saved = reply.startup_enabled
        {
            startupEnabled = saved
        }
    }
    func stopAll() async {
        guard !working else { return }
    ...
    func billing() {
        NSWorkspace.shared.open(
            URL(
                string: paid
                    ? "https://modeluplink.com/dashboard/#billing-panel"
                    : "https://modeluplink.com/dashboard/?plan=agent#billing-panel")!)
    }
}
