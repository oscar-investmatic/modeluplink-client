...
$Profile=Join-Path $env:RUNNER_TEMP 'Model Uplink profile test'
$Models=Join-Path $env:RUNNER_TEMP 'Model Uplink models test'
$env:MODELUPLINK_CONFIG_DIR=$Profile
...
Run-Installer $Installer $Arguments
if(-not (Test-Path "$InstallDir\modeluplink-app.exe")){throw 'Desktop app was not installed'}
if(-not (Test-Path "$Profile\config.json")){throw 'Installer finalization did not create protected settings'}
# Exercise an upgrade with no active endpoint; it must preserve the user profile.
Run-Installer $Installer $Arguments
Run-Installer "$InstallDir\unins000.exe" '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART'
if(Test-Path "$InstallDir\modeluplink.exe"){throw 'Uninstaller left the helper behind'}
if(Test-Path "$Profile\config.json"){throw 'Uninstaller left local credentials behind'}
